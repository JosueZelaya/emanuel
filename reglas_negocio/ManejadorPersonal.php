...
abstract class ManejadorPersonal{
	...
	public static function getUsuario($login){
		if (ereg("[^A-Za-z0-9]+",$login)) {	//EVITAR QUE EN EL LOGIN APAREZCAN CARACTERES ESPECIALES
			...
			$usuario->setHabilitado($respuesta['habilitado']);
                        ...
                        if($respuesta['fecha_nacimiento']!="")
                            $usuario->setFechaNacimiento(date("d-m-Y", strtotime($respuesta['fecha_nacimiento'])));
                        if($respuesta['fecha_conversion']!="")
                            $usuario->setFechaConversion(date("d-m-Y", strtotime($respuesta['fecha_conversion'])));
                        if($respuesta['fecha_bautismo']!="")
                            $usuario->setFechaBautismo(date("d-m-Y", strtotime($respuesta['fecha_bautismo'])));
			return $usuario;
		}		
	}
        ...
        public static function buscarUsuario($buscarComo,$campo){
			...
			throw new Exception("¡Nombre invalido!");	
                ...
                if($campo=="nombre_completo")
                    $sql_consulta = "SELECT * FROM personas WHERE nombres ILIKE '%".$buscarComo."%' OR apellidos ILIKE '%".$buscarComo."%' LIMIT 25";                    
        ...
        public static function buscarUsuarioOriginal($buscarComo,$campo){            
            $datos = array();
            $miembros = array();
            if (ereg("[^A-Za-z0-9]+",$buscarComo)) {	//EVITAR QUE EN EL LOGIN APAREZCAN CARACTERES ESPECIALES
			throw new Exception("¡Nombre invalido!");	
                    ...
                    if($persona->getNombres()!=="" && $persona->getApellidos()!==""){
                        $consulta = "INSERT INTO personas (nombres,apellidos, correo,telefono,direccion,fecha_nacimiento,fecha_conversion,fecha_bautismo) VALUES ('".
                                ...
                                if($persona->getFechaNacimiento()=="")
                                    ...
                                    $consulta=$consulta.",'".$persona->getFechaNacimiento()."'";
                                if($persona->getFechaConversion()=="")
                                    $consulta=$consulta.",NULL";
                                else
                                    $consulta=$consulta.",'".$persona->getFechaConversion()."'";
                                if($persona->getFechaBautismo()=="")
                                    $consulta=$consulta.",NULL";
                                else
                                    $consulta=$consulta.",'".$persona->getFechaBautismo()."'";
                                $consulta = $consulta.")";
